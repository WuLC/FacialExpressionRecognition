...
def calibrateImge(imgpath):
    ...
    #td2=time.time()
    #print('Time in calibrateImage: %fs'%(td2-td1))
...
def calibrateImageWithArrayInput(img):
    ...
    imgcv_gray=np.array(rtimg)
    rectPoint=[]
    rectPoint.append((dets[0].left(),dets[0].top()))
    rectPoint.append((dets[0].right(),dets[0].bottom()))
...
def preprocessImage(img):
    ...
    features = {}
    detected, img_gray, rectPoints = calibrateImageWithArrayInput(img)
    features['detected'] = detected
    if detected: # detect human face
        #rescaleimg, gf, geo_features, pf, eyepatch, foreheadpatch, mouthpath, innerface=getLandMarkFeatures_and_ImgPatches(img_gray, True, True, True)
        rescaleimg, gf, geo_features, pf, eyepatch, foreheadpatch, mouthpatch, innerface=getLandMarkFeatures_and_ImgPatches(img_gray, False, False, True)
        ...
        if cropped:
            rectPoints = [list(point) for point in rectPoints]
            rectPoints[0][0] += left_top[1]
            rectPoints[0][1] += left_top[0]
            rectPoints[1][0] += left_top[1]
            rectPoints[1][1] += left_top[0]
            rectPoints = [tuple(point) for point in rectPoints]

        rescaleimg = rescaleimg.reshape(1, 128, 128, 1) # reshape face image as input to model
        features['rescaleimg'] = rescaleimg
        features['rectPoints'] = rectPoints
        features['geo_features'] = geo_features
        features['eyepatch'] = eyepatch
        features['foreheadpatch'] = foreheadpatch
        features['mouthpatch'] = mouthpatch
        features['innerface'] = innerface
    return features
